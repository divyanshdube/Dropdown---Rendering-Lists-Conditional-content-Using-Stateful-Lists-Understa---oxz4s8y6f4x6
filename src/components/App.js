...
function App() {
  const [stateSet, isStateSet] = useState(0);
  const [citySet, isCitySet] = useState(0);
  const [landmarkSet, isLandmarkSet] = useState(0);

  const changeState = (event) => {
    isStateSet(event.target.value);
    isCitySet(0);
    isLandmarkSet(0);
  };

  const changeCity = (event) => {
    isCitySet(event.target.value);
    isLandmarkSet(0);
  };

  const changeLandmark = (event) => {
    isLandmarkSet(event.target.value);
  };
    ...
    <div id="main">
      ...
      <div id="city-name">
        <select id="city" onChange={changeCity}>
          {states[stateSet].city.map((value, index) => (
      ...
      <div id="landmark-name">
        <select id="landmark" onChange={changeLandmark}>
          <br />
          {states[stateSet].city[citySet].landmarks.map((value, index) => (
            <option value={index}>{value.name}</option>
          ))}
        </select>
      </div>

      <div id="state-title">{states[stateSet].name}</div>
      <div id="state-description">{states[stateSet].description}</div>
      <br />

      <div id="city-title">{states[stateSet].city[citySet].name}</div>
      <div id="city-description">
        {states[stateSet].city[citySet].description}
      </div>
      <br />

      <div id="landmark-title">
        {states[stateSet].city[citySet].landmarks[landmarkSet].name}
      </div>
      <div id="landmark-description">
        {states[stateSet].city[citySet].landmarks[landmarkSet].description}
      </div>
      <br />
    </div>
  );
